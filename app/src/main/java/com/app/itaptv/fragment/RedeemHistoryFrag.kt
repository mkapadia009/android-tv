...
class RedeemHistoryFrag : Fragment() {
    ...
    private val arrayListRedeemHistory = ArrayList<EarnHistoryData>()
    ...
    companion object {
        @JvmStatic
        fun newInstance(): RedeemHistoryFrag {
            return RedeemHistoryFrag()
        }
    }
    ...
    private fun init() {
        ...
        rvEarnHistory = rootView.findViewById(R.id.rvRedeemHistory)
        tvLabelNoHistoryFound = rootView.findViewById(R.id.tvLabelNoRedeemHistoryFound)
        historyFlag = WalletActivity.FLAG_REDEEM_HISTORY
        ...
        if (success) {
            if (jsonArray != null) {
                try {
                    ...
                    //arrayListRedeemHistory.clear()
                    for (i in 0 until jsonArray.length()) {
                        val earnHistoryData =
                            EarnHistoryData(jsonArray.getJSONObject(i), historyFlag)
                        arrayListRedeemHistory.add(earnHistoryData)
                    }
                    adapter.notifyDataSetChanged()
                    loading = false
                    page++
    ...
    private fun initializeRedeemPagination() {
        adapter.notifyDataSetChanged()
